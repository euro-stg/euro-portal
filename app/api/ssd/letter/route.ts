  ...
  try {
    ...
    const [rows, total] = await Promise.all([
      db.ssdLetter.findMany({
        where,
        select: {
          id: true, letterNo: true, title: true, status: true, fileDraft: true, fileFinal: true,
...
export async function POST(request: Request) {
  try {
    ...
    if (!content?.trim())     return NextResponse.json({ message: "Isi surat wajib diisi" }, { status: 400 });
