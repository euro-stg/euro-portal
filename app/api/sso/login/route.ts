...
export async function POST(request: Request) {
  ...
  function log(status: "SUCCESS" | "FAILED", opts: { appTokenId?: string | null; userId?: string | null; statusCode: number; reason?: string }) {
    writeApiLog({ method: "POST", endpoint: ENDPOINT, status, ip, duration: Date.now() - start, ...opts });
  }
  ...
  try {
    ...
    if (!employeeId || !password) {
      log("FAILED", { appTokenId: appRecord.id, statusCode: 400, reason: "employeeId dan password wajib diisi" });
    ...
    if (!user || !user.password) {
      log("FAILED", { appTokenId: appRecord.id, statusCode: 401, reason: "Kredensial tidak valid" });
    ...
    if (user.resignDate) {
      ...
      if (resignDay <= today) {
        log("FAILED", { appTokenId: appRecord.id, userId: user.id, statusCode: 403, reason: "Akun tidak aktif (resign)" });
        return NextResponse.json({ error: "Akun tidak aktif" }, { status: 403 });
      }
    }
    if (user.status === "inactive") {
      log("FAILED", { appTokenId: appRecord.id, userId: user.id, statusCode: 403, reason: "Akun tidak aktif" });
      return NextResponse.json({ error: "Akun tidak aktif" }, { status: 403 });
    }

    const valid = await bcrypt.compare(password, user.password);
    if (!valid) {
      log("FAILED", { appTokenId: appRecord.id, userId: user.id, statusCode: 401, reason: "Password salah" });
      return NextResponse.json({ error: "Kredensial tidak valid" }, { status: 401 });
    }
    ...
    log("SUCCESS", { appTokenId: appRecord.id, userId: user.id, statusCode: 200 });
