...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Check permission
    if (!hasPermission(claims.permissions, "GET_USERS")) {
    ...
    // Lookup app record — revocation check + active check
    ...
    const [users, total] = await Promise.all([
      db.user.findMany({
        where,
        select: {
          id: true, employeeId: true, name: true, lastName: true,
          email: true, phone: true, mobilePhone: true,
          gender: true, birthPlace: true, birthDate: true,
          address: true, religion: true, bloodType: true,
          maritalStatus: true, identityType: true, identityNumber: true,
          jobPositionId: true, jobPositionName: true, jobLevel: true,
          organizationId: true, organizationName: true,
          branchId: true, branchName: true,
          employmentStatus: true, age: true,
          joinDate: true, resignDate: true,
