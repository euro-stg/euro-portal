...
const ENDPOINT = "/api/sso/branches";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!hasPermission(claims.permissions, "GET_BRANCHES")) {
      ...
      where: search
        ? {
            OR: [
              ...
              { regencyName:  { contains: search, mode: "insensitive" } },
    ...
    return NextResponse.json({ data: branches, total: branches.length });
  } catch (err) {
    console.error("[SSO Branches]", err);
