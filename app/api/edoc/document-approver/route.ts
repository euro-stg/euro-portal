...
// Document Approver adalah role global E Document, independen dari Role/Module Portal.
...
export async function GET() {
  try {
    ...
    const rows = await db.eDocDocumentApprover.findMany({
...
export async function POST(request: Request) {
  try {
    ...
    const row = await db.eDocDocumentApprover.upsert({
      where: { userId },
